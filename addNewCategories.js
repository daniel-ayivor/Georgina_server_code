...
async function addNewCategories() {
  try {
    ...
    for (const categoryData of newCategoriesData) {
      ...
      const existingCategory = await Category.findOne({ where: { slug: categoryInfo.slug } });
      if (existingCategory) {
      ...
      for (const subcategoryData of subcategories) {
        ...
        if (nestedSubcategories) {
          for (const nestedSubcategoryData of nestedSubcategories) {
            // For now, we'll create these as regular subcategories under the main category
            // since the database structure expects parent_id to reference categories table
            const nestedSubcategory = await SubCategory.create({
              ...nestedSubcategoryData,
              parentId: category.id
            });
            console.log(`Created subcategory: ${nestedSubcategory.name}`);
          }
        }
      }
    }

    console.log('New categories and subcategories added successfully!');
    process.exit(0);
  } catch (error) {
